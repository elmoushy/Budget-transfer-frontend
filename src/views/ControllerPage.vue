      ...
      <!-- <div class="add-section glass-panel">
        ...
        <form @submit.prevent="addCurrency" class="add-form">
          <div class="form-group">
            <label for="currency-select">{{ isArabic ? 'اختر العملة' : 'Select Currency' }}</label>
            <select id="currency-select" v-model="selectedCurrency" class="form-select" required>
              <option value="" disabled>
                {{ isArabic ? 'اختر عملة...' : 'Choose a currency...' }}
              </option>
              <option
                ...
                :value="currency"
              >
                {{ currency.currency_icon }} {{ currency.currency_name }} ({{ currency.country }})
              </option>
            </select>
          </div>
          <div class="form-group preview-group" v-if="selectedCurrency">
            <label>{{ isArabic ? 'معاينة' : 'Preview' }}</label>
            <div class="currency-preview">
              <span class="preview-icon">{{ selectedCurrency.currency_icon }}</span>
              <span class="preview-name">{{ selectedCurrency.currency_name }}</span>
              <span class="preview-country">{{ selectedCurrency.country }}</span>
            </div>
          </div>
          <button
            type="submit"
      ...
      <div class="list-section glass-panel">
        <div class="section-header">
          <h3 class="section-title">
            ...
            {{ isArabic ? 'العمله المتاحة' : 'Available Currency' }}
          </h3>
    ...
    <div v-if="showEditCurrencyModal" class="modal-overlay" @click="closeEditCurrencyModal">
      <div class="modal-content" @click.stop>
        ...
        <form @submit.prevent="updateCurrency" class="modal-form">
          <div class="form-group">
            <label>{{ isArabic ? 'اختر العملة' : 'Select Currency' }}</label>
            <select
              v-model="editingCurrency.selectedOption"
              class="form-select"
              @change="updateEditingCurrency"
              required
            >
              <option value="" disabled>
                {{ isArabic ? 'اختر عملة...' : 'Choose a currency...' }}
          ...
          <div class="form-group preview-group" v-if="editingCurrency.selectedOption">
            <label>{{ isArabic ? 'معاينة' : 'Preview' }}</label>
            <div class="currency-preview">
              <span class="preview-icon">{{ editingCurrency.selectedOption.currency_icon }}</span>
              <span class="preview-name">{{ editingCurrency.selectedOption.currency_name }}</span>
              <span class="preview-country">{{ editingCurrency.selectedOption.country }}</span>
            </div>
          </div>
          <div class="modal-actions">
            ...
            <button type="submit" class="btn-primary" :disabled="!editingCurrency.selectedOption">
              {{ isArabic ? 'حفظ' : 'Save' }}
            </button>
          </div>
        </form>
...
interface EditingCurrency extends Partial<Currency> {
...
// Currency options from JSON
const currencyOptions = ref<CurrencyOption[]>(currencyData.currencies)
...
const editingCurrency = ref<EditingCurrency>({})
...
const addCurrency = async () => {
  if (!selectedCurrency.value) {
    showFeedback(isArabic.value ? 'يرجى اختيار عملة' : 'Please select a currency', false)
    return
  }

  try {
    const currencyToAdd = {
      name: selectedCurrency.value.currency_name,
      icon: selectedCurrency.value.currency_icon,
    }
    await controllerService.createCurrency(currencyToAdd)
...
const editCurrency = (currency: Currency) => {
  // Find the matching currency option based on name and icon
  const matchingOption = currencyOptions.value.find(
    (option) => option.currency_name === currency.name && option.currency_icon === currency.icon,
  )

  editingCurrency.value = {
    ...currency,
    selectedOption: matchingOption || null,
  }
  showEditCurrencyModal.value = true
}

const updateEditingCurrency = () => {
  if (editingCurrency.value.selectedOption) {
    editingCurrency.value.name = editingCurrency.value.selectedOption.currency_name
    editingCurrency.value.icon = editingCurrency.value.selectedOption.currency_icon
  }
}

const updateCurrency = async () => {
  try {
    if (editingCurrency.value.id && editingCurrency.value.selectedOption) {
      const updatedCurrency = {
        name: editingCurrency.value.selectedOption.currency_name,
        icon: editingCurrency.value.selectedOption.currency_icon,
      }
      await controllerService.updateCurrency(editingCurrency.value.id, updatedCurrency)
...
const updateRoute = async () => {
  try {
    if (editingRoute.value.id) {
      ...
      }, 1000) // Wait 1 second to show the success message before refresh
    }
  } catch (error) {
...
.form-select {
  ...
  cursor: pointer;
  appearance: none;
  background-image: url("data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='%236d1a36' viewBox='0 0 16 16'%3E%3Cpath d='M8 13.1l-8-8 2.1-2.1 5.9 5.9 5.9-5.9 2.1 2.1z'/%3E%3C/svg%3E");
...
.controller-page.dark-theme .form-select {
  background: rgba(36, 23, 38, 0.8);
...
.controller-page.dark-theme .form-select {
  background-image: url("data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' fill='%23f8e9f0' viewBox='0 0 16 16'%3E%3Cpath d='M8 13.1l-8-8 2.1-2.1 5.9 5.9 5.9-5.9 2.1 2.1z'/%3E%3C/svg%3E");
}
...
.controller-page.dark-theme .form-select option {
...
.form-select:hover {
  border-color: #e14b6a;
  box-shadow: 0 2px 8px rgba(225, 75, 106, 0.1);
}
...
.controller-page.dark-theme .currency-preview {
  background: rgba(225, 75, 106, 0.1);
...
.controller-page.dark-theme .preview-name {
  color: #f8e9f0;
}
...
.controller-page.dark-theme .preview-country {
  color: #f8e9f0;
}
