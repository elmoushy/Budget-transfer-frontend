<template>
  <div class="approval-page" :class="{ 'dark-mode': isDarkMode }">
    ...
    <div class="toolbar">
      <h1 class="page-title">
        {{ isArabic ? 'التعزيزات قيد الاعتماد' : 'Enhancements Pending Approval' }}
      </h1>
...
defineOptions({
  name: 'EnhancementsPendingApprovalPage',
})
...
function showConfirmationModal(type: 'approve' | 'reject', rowsToAction: RowData[]) {
  ...
  if (type === 'approve') {
    ...
    confirmModalMessage.value = isArabic.value
      ? `هل أنت متأكد من موافقتك على ${count} من التعزيزات؟`
      : `Are you sure you want to approve ${count} enhancement${count === 1 ? '' : 's'}?`
  } else {
    confirmModalTitle.value = isArabic.value ? 'تأكيد الرفض' : 'Confirm Rejection'
    confirmModalMessage.value = isArabic.value
      ? `هل أنت متأكد من رفضك لـ ${count} من التعزيزات؟`
      : `Are you sure you want to reject ${count} enhancement${count === 1 ? '' : 's'}?`
  }
