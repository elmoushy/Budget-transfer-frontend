<template>
  <div class="approval-page" :class="{ 'dark-mode': isDarkMode }">
    ...
    <div class="toolbar">
      <h1 class="page-title">
        {{ isArabic ? 'التسويات قيد الاعتماد' : 'Settlements Pending Approval' }}
      </h1>
...
function showConfirmationModal(type: 'approve' | 'reject', rowsToAction: RowData[]) {
  ...
  if (type === 'approve') {
    ...
    confirmModalMessage.value = isArabic.value
      ? `هل أنت متأكد من موافقتك على ${count} من التسويات؟`
      : `Are you sure you want to approve ${count} settlement${count === 1 ? '' : 's'}?`
  } else {
    confirmModalTitle.value = isArabic.value ? 'تأكيد الرفض' : 'Confirm Rejection'
    confirmModalMessage.value = isArabic.value
      ? `هل أنت متأكد من رفضك لـ ${count} من التسويات؟`
      : `Are you sure you want to reject ${count} settlement${count === 1 ? '' : 's'}?`
  }
